...
public class DashboardService
{
    ...
    public async Task<DashboardDto> GetDashboardAsync()
    {
        ...
        return new DashboardDto
        {
            ...
            VentasMes               = ventasMes,
            FacturasPendientesDian  = facturasPendientesDian,
    ...
    public async Task<SuperAdminDashboardDto> GetSuperAdminDashboardAsync()
    {
        ...
        var inicioMesAnt    = inicioMesActual.AddMonths(-1);
        ...
        var ingresosMes = await _db.Pagos
            .Where(p => p.Estado == "Completado" && p.Fecha >= inicioMesActual)
            .SumAsync(p => (decimal?)p.Monto) ?? 0m;

        var ingresosMesAnt = await _db.Pagos
            .Where(p => p.Estado == "Completado" && p.Fecha >= inicioMesAnt && p.Fecha < inicioMesActual)
            .SumAsync(p => (decimal?)p.Monto) ?? 0m;
        ...
        // Renewal Rate: Tenants with a completed payment in the last 30 days
        var countConPagoReciente = await _db.Pagos
            .Where(p => p.Estado == "Completado" && p.Fecha >= hace30Dias)
            .Select(p => p.TenantId)
            .Distinct()
            .CountAsync();

        var totalActivos = tenants.Count(t => t.Activo);
        var tasaRenovacion = totalActivos > 0 ? (double)countConPagoReciente / totalActivos : 0;

        return new SuperAdminDashboardDto
        {
            TotalTenants         = tenants.Count,
            TenantsActivos       = totalActivos,
    ...
    public async Task<(decimal RecaudoMes, decimal IngresosMesAnt, decimal PagosPendientes, double TasaRenovacion)> GetPagosSummaryAsync()
    {
        var ahora           = DateTime.UtcNow;
        var inicioMesActual = new DateTime(ahora.Year, ahora.Month, 1);
        var inicioMesAnt    = inicioMesActual.AddMonths(-1);
        ...
        var totalActivos = await _db.Tenants.CountAsync(t => t.Activo);
        var tasaRenovacion = totalActivos > 0 ? (double)countConPagoReciente / totalActivos : 0;

        return (ingresosMes, ingresosMesAnt, pagosPendientes, tasaRenovacion);
    }
}

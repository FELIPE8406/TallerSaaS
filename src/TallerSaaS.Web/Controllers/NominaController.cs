...
public class NominaController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        ViewBag.Mechanics = await _userProvider.GetMechanicsAsync(tenantId ?? Guid.Empty);
        return View();
    }
    ...
    {
        // Forzar periodo actual si viene vacío (Fix Filtros)
    ...
    public async Task<IActionResult> ReportarDian(Guid id)
    {
        ...
        if (result)
            return Json(new { success = true, message = "Nómina reportada exitosamente ante la DIAN." });

        return Json(new { success = false, message = "No se pudo reportar la nómina." });
    }
    ...
    public async Task<IActionResult> DescargarPdf(Guid id)
    {
        ...
        string? logoPath = !string.IsNullOrEmpty(tenant?.Logo) ? Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "logos", tenant.Logo) : null;
